...
export class VoucherService {
  ...
  async createVoucher(
    ...
    const rules = ruleIds
      ? await this.eligibilityRuleRepository.find({
          where: { id: In(ruleIds) },
        })
      : [];
  ...
  async updateVoucher(voucherId: number, updateData: any, rules: number[]) {
    ...
    if (rules && rules.length > 0) {
      const ruleEntities = await Promise.all(
        rules.map((ruleId) =>
  ...
  private _canRedeemVoucher(
    ...
    if (
      voucher.users &&
      voucher.users.map((userVoucher) => userVoucher.user.id).includes(user.id)
    ) {
    ...
    for (const rule of rules) {
      ...
      switch (rule.userProperty) {
        case UserProperties.ROLES:
          if (user.roles !== rule.value) errorMessage = rule.description;
          break;

        case UserProperties.REFERRED_BY:
          if (!user.referrer || Number(rule.value) !== user.referrer.id) {
