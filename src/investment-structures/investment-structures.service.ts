...
export class InvestmentStructuresService {
  ...
  async create(createInvestmentStructureDto: CreateInvestmentStructureDto) {
    return await this.investmentStructureRepository.save(
      createInvestmentStructureDto,
    );
  }
  ...
  async findOne(id: number) {
    const investment = await this.investmentStructureRepository.findOneBy({
      id,
    });
    if (!investment) {
      throw new NotFoundException(
        `Investment structure with id ${id} not found`,
      );
    }
    return investment;
  }
  ...
  async update(
    id: number,
    updateInvestmentStructureDto: UpdateInvestmentStructureDto,
  ) {
    ...
    if (Object.keys(updates).length > 0)
      await this.investmentStructureRepository.update(
        id,
        updateInvestmentStructureDto,
      );
